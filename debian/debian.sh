...
if [ "$SESSIONEXISTS" == "" ]; then
	...
	tmux send-keys -t "LynisParse" "grep suggestion /var/log/lynis-report.dat | less"
	
	# Fifth window for checking users
	tmux new-window -t $SESSIONW:4 -n "CheckUsers"
	tmux send-keys -t "CheckUsers" "./userSort.sh | less" C-m

	# Sixth window for checking services
	tmux new-window -t $SESSIONW:5 -n "CheckServices"
	tmux send-keys -t "CheckServices" "./serviceSort.sh | less" C-m
